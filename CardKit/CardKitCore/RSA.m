...
static NSString *__base64_encode_data(NSData *data) {
  data = [data base64EncodedDataWithOptions:0];
  return [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
}

static NSData *__base64_decode(NSString *str){
...
+ (SecKeyRef)addPublicKey:(NSString *)key{
  ...
  NSData *data = __base64_decode(key);
...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
  NSData *data = [RSA _encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey];
  return __base64_encode_data(data);
}
